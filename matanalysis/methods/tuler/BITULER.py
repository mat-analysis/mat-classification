...
def BITULER_read(dir_path, res_path='.', prefix='', save_results=False, n_jobs=-1, random_state=42, rounds=10, label_poi='poi', geohash=False, geo_precision=30):
    
    # Load Data - Tarlis:
    df_train, df_test = loadTrajectories(dir_path, prefix)
    
    return BITULER(df_train, df_test, res_path, prefix, save_results, n_jobs, random_state, rounds, label_poi, geohash, geo_precision)
    

def BITULER(df_train, df_test, res_path='.', prefix='', save_results=False, n_jobs=-1, random_state=42, rounds=10, label_poi='poi', geohash=False, geo_precision=30):
...
#        rounds = 10
        ...
        # Prediction
        # ---------------------------------------------------------------------------------
        model = ModelContainer(bituler, y_test, X_test, cls_history=evaluate_report, approach='BITULER', le=dic_parameters['encode_y'])
